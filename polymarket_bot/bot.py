...
class PolymarketBot:
    ...
    async def _run_loop(self):
        ...
        while self._running:
            try:
                ...
                if (datetime.now(UTC) - last_stats_time).total_seconds() >= self._stats_interval:
    ...
    async def _refresh_markets(self):
        should_full_refresh = (
            ...
            (datetime.now(UTC) - self._last_full_scan).total_seconds() > 300
        )
        ...
        if should_full_refresh:
            ...
            self._last_full_scan = datetime.now(UTC)
